...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.selectedTitleColor = [UIColor colorWithRed:254.f/255 green:143.f/255 blue:63.f/255 alpha:1];
...
- (void)layoutSubviews
{
    ...
    if ([self viewWithTag:10010]) {
        ...
        self.scrollView.width = self.width - more.width;
    } else {
        self.scrollView.frame = self.bounds;
    }
    
    self.bottomBorder.height = 1;
    self.bottomBorder.width = self.width;
    self.bottomBorder.left = 0;
    self.bottomBorder.bottom = self.height;
    
    self.cursor.hidden = !self.showCusor;
    self.cursor.height = 3;
    self.cursor.bottom = self.height;
    ...
    for (HLYScrollTabbarItem *item in self.contentViews) {
        item.left = currentX;
        item.height = self.scrollView.height - 2 * self.verticalPadding;
        item.centerY = self.scrollView.height / 2;
        if (self.tabbarContentMode != HLYScrollTabbarModeFlexibleWidth) {
            item.width = itemWidth;
        }
        currentX += item.width + (self.horizPaddingMode == HLYHorizPaddingModeMargin?0:self.horizPadding);
        
        if (self.showBorder && i != titleCount - 1) {
            UIView *border = [self.borders objectAtIndex:i];
            border.left = item.right;
            border.width = 1;
            border.height = self.height;
        }
        
        if (self.cursor.width != item.width) {
            self.cursor.width = item.width;
        }
        
        i++;
    }
    self.scrollView.contentSize = CGSizeMake(currentX, self.scrollView.height);
}
...
- (void)tabbarDidTappedAtIndex:(NSInteger)index withUserInteraction:(BOOL)withUserInteraction
{
    ...
    CGFloat leftOffset = currentItem.left - self.scrollView.contentOffset.x;
    CGFloat rightOffset = currentItem.right - self.scrollView.contentOffset.x - ([self viewWithTag:10010] ? [self viewWithTag:10010].left : self.scrollView.right);
    if (leftOffset < 0) {
        [self.scrollView setContentOffset:CGPointMake(currentItem.left, 0) animated:YES];
    ...
    //
    self.cursor.left = index * currentItem.width;
}
